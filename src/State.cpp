...
const bool State::canMove(direction dir) const
{
    ...
    case direction::right:
        return xCoord(head) < width - 1 && val(dir) <= EMPTY;
    case direction::left:
        return xCoord(head) > 0 && val(dir) <= EMPTY;
    case direction::up:
        return yCoord(head) < height - 1 && val(dir) <= EMPTY;
    default:
        return yCoord(head) > 0 && val(dir) <= EMPTY;
    }
}
...
int &State::val(int x, int y) { return val(point(x, y)); }
const int &State::val(int x, int y) const { return val(point(x, y)); }
